...
local function draw_freq_screen()
    ...
    for band=1,#(bands) do
        -- draw band letter
        lcd.drawText(xpos[band]+7, 1, bands[band], SMLSIZE)
        -- draw frequencies
        for chan=1,8 do
            lcd.drawText(xpos[band], ypos[chan], freqs[band][chan], SMLSIZE)
        end
    end
end
...
local function draw_pos_screen()
    ...
    for band=1,#(bands) do
        -- draw band letter
        lcd.drawText(1, ypos[band]+2, bands[band], SMLSIZE)
        -- draw channel boxes
        for chan=1,8 do
